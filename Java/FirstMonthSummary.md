...
# 4.0 处理文本
## 4.1 字符串操作
- String类的indexOf可以用来查找子串，然后再截取子串。如果没有找到，indexOf返回-1。
- subString取得原字符串其中的一段。
- StringTokenizer按照指定字符分割字符串，切割长字符串的时候，效率更高。
- +连接字符
- StringBuilder对象用来连接字符串对象。清空StringBuilder时候，直接set length为0

## 4.2 有限状态机
状态转换图
	- 转换函数，接受一个状态和输入符号，输出下一个状态
	- 状态转换表中每行表示一个状态，每列表示一个输入字符
	- 可以用一个二位数组来记录状态转换表：第一个维度是所有可能的状态，第二维度是所有可能的事件，二位数组中的值是目标状态。

如果从同一个状态接受同样的输入后可以任意到达多个不同的状态，属于非确定有限状态机。从一个状态接收一个输入后只能到达某一个状态，属于确定有限状态机。正则表达式可以写成对应的有限状态机。

任何非确定有限状态机都可以转换成确定有限状态机，方法是幂级构造。
- 写出非确定有限状态机的状态转换表
- 构建新的状态使其变成确定状态机
- 合并状态，最小化确定状态机

确定有限状态机需要确定初始状态，状态转移函数，结束状态。

可以把Trie树看成是有限状态转换机，接收字符，转换出来的也是字符。
